...
from layers import layer, residual, conv2d, layer2d, pool, pool_frames, gru_last, flatten, flatten_multi, dense, dense_multi, weights, bias, get_summaries
import threading
...
def prediction_network(x_h, x_m, test=False):
    with tf.variable_scope("network") as sc:
        ...
        decoded_future = gru_last(combined, 512 * 2, 1, batch_size, "lstm_future")

        return tf.add(tf.nn.l2_loss(decoded_past, past), tf.nn.l2_loss(decoded_future, future))
...
with tf.device("/cpu:0"):
    train_runner = PredictionRunner(lambda : get_batches(CHALAP, batch_size, seq_len * 2, 1, 3, 0, 0, 0, True))
    val_runner = PredictionRunner(lambda : get_batches(CHALAP, batch_size, seq_len * 2, 4, 5, 0, 0, 0, True))
    train_x_h, train_x_m, train_o_h_past, train_o_m_past, train_o_m_future, train_o_m_future = train_runner.get_inputs()
    val_x_h, val_x_m, val_o_h_past, val_o_m_past, val_o_m_future, val_o_m_future = val_runner.get_inputs()
...
for epoch in range(20):
    ...
    for i in range(1663):
        vloss, _ = sess.run([val_loss])
        print("step %d, validation loss %g"%(i, tloss), flush=True)
