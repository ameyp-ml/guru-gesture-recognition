...
def read_video(sample_num):
    base_dir = "/home/aparulekar-ms/chalap"
...
def read_segments(sample_num, interval_size):
    base_dir = "/home/aparulekar-ms/chalap"
    ...
    with open(labels_path, 'r') as f:
        for l in f:
            ...
            segments.append((np.stack(hand, axis=0),
                             np.stack(main, axis=0),
                             logit(label)))
...
def get_pretrain_batches(size, interval_size, infinite=False):
    sample_num = 1
    segments = []
    segment = None
    batch = []
    ...
    while True:
        ...
        if sample_num > 470:
            if infinite:
                ...
                return
        if len(segments) == 0:
            segments = read_segments(sample_num, interval_size)
            sample_num += 1
        if segment == None:
            segment = segments[0]
            segments = segments[1:]
...
def get_batches(size, interval_size, start, end, infinite=False):
    sample_num = start
    segments = []
    segment = None
    batch = []
    ...
    while True:
        ...
        rem = size - len(batch)
        batch = batch + segments[:rem]
        segments = segments[rem:]
